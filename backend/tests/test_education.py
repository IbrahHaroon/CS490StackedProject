...
class TestCreateEducation:
    ...
    def test_returns_education_object(self, session, user):
        edu = create_education(session, user.user_id, "Bachelor's", "Computer Science", "State U", "1 Campus Dr", "NJ", 8901)
        assert edu is not None

    def test_education_id_assigned(self, session, user):
        edu = create_education(session, user.user_id, "Bachelor's", "CS", "State U", "1 Campus Dr", "NJ", 8901)
        assert edu.education_id is not None
        assert edu.education_id >= 1

    def test_degree_stored_correctly(self, session, user):
        edu = create_education(session, user.user_id, "Master's", "Data Science", "Tech U", "2 Uni Ave", "NY", 10001)
        assert edu.degree == "Data Science"
        assert edu.highest_education == "Master's"

    def test_school_stored_correctly(self, session, user):
        edu = create_education(session, user.user_id, "Bachelor's", "Math", "City College", "3 College Rd", "CA", 90210)
        assert edu.school_or_college == "City College"

    def test_address_id_created_and_linked(self, session, user):
        edu = create_education(session, user.user_id, "PhD", "Physics", "Research U", "4 Lab Blvd", "MA", 2101)
        assert edu.address_id is not None

    def test_user_id_linked_correctly(self, session, user):
        edu = create_education(session, user.user_id, "Associate's", "Business", "Comm College", "5 Main St", "TX", 73301)
        assert edu.user_id == user.user_id

    def test_one_user_can_have_multiple_degrees(self, session, user):
        edu1 = create_education(session, user.user_id, "Bachelor's", "CS", "State U", "1 St", "NJ", 1000)
        edu2 = create_education(session, user.user_id, "Master's", "AI", "Tech U", "2 Ave", "NY", 2000)
...
class TestGetEducation:
    ...
    def test_fields_match_after_fetch(self, session, user):
        edu = create_education(session, user.user_id, "PhD", "ML", "Research U", "6 Sci Blvd", "WA", 98101)
        fetched = get_education(session, edu.education_id)
    ...
    def test_school_matches_after_fetch(self, session, user):
        edu = create_education(session, user.user_id, "Bachelor's", "Art", "Art Institute", "7 Art Ln", "IL", 60601)
        fetched = get_education(session, edu.education_id)
