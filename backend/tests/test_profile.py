...
class TestCreateProfile:
    ...
    def test_returns_profile_object(self, session, user):
        profile = create_profile(session, user.user_id, "Jane", "Doe", date(1990, 1, 1), "1 Main St", "NJ", 8534)
    ...
    def test_dob_stored_correctly(self, session, user):
        dob = date(1995, 6, 15)
        profile = create_profile(session, user.user_id, "John", "Smith", dob, "2 Oak Ave", "NY", 10001)
        assert profile.dob == dob

    def test_address_id_created_and_linked(self, session, user):
        profile = create_profile(session, user.user_id, "Jane", "Doe", date(1990, 1, 1), "3 Pine Rd", "CA", 90210)
        assert profile.address_id is not None

    def test_user_id_linked_correctly(self, session, user):
        profile = create_profile(session, user.user_id, "Jane", "Doe", date(1990, 1, 1), "4 Elm St", "TX", 73301)
...
class TestGetProfile:
    ...
    def test_fields_match_after_fetch(self, session, user):
        profile = create_profile(session, user.user_id, "Alice", "Brown", date(1988, 3, 22), "5 Birch Ln", "FL", 33101)
        fetched = get_profile(session, profile.profile_id)
    ...
    def test_dob_matches_after_fetch(self, session, user):
        dob = date(2000, 12, 31)
        profile = create_profile(session, user.user_id, "Bob", "Green", dob, "6 Cedar Ct", "WA", 98101)
        fetched = get_profile(session, profile.profile_id)
...
class TestUpdateProfile:
    ...
    def test_other_profiles_unaffected(self, session):
        ...
        p1 = create_profile(session, u1.user_id, "Alice", "A", date(1990, 1, 1), "1 St", "NJ", 1000)
        p2 = create_profile(session, u2.user_id, "Bob", "B", date(1991, 2, 2), "2 St", "NY", 2000)
